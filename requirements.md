...
| レイヤ | 項目 | 選定技術 | 選定理由 |
|--------|------|----------|----------|
| フロントエンド | フレームワーク | React 19.x（Vite 経由） | コンポーネント設計の学習、エコシステムが豊富 |
| フロントエンド | 言語 | TypeScript 6.0.x | 型安全なドメインモデル定義 |
| フロントエンド | ビルド | Vite 8.x | 高速な開発サーバー・HMR |
...
| バックエンド | ランタイム | Java 21（Microsoft OpenJDK） | 型安全・エンタープライズ標準 |
| バックエンド | フレームワーク | Spring Boot 4.0.0 | 実務で広く使われる Java Web フレームワーク |
| バックエンド | 言語 | Java | Spring エコシステムとの親和性が高い |
| バックエンド | ビルドツール | Gradle 8.14 | 柔軟なビルド設定・依存管理 |
| データベース | RDBMS | PostgreSQL 18.4 | オープンソースで機能が豊富な RDBMS |
